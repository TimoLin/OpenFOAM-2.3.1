...
#include "meanVelocityProfileSource.H"
...
namespace fv
{
    defineTypeNameAndDebug(meanVelocityProfileSource, 0);
...
void Foam::fv::meanVelocityProfileSource::writeProps
(
    const scalar gradP
) const
{
...
Foam::fv::meanVelocityProfileSource::meanVelocityProfileSource
(
...
:
    ...
    r_(List<scalar> (coeffs_.lookup("r"))),
    velProfile_(List<scalar> (coeffs_.lookup("velProfile"))),
    rmsProfile_(List<scalar> (coeffs_.lookup("rmsProfile"))),
    invAPtr_(NULL)
{
...
void Foam::fv::meanVelocityProfileSource::correct(volVectorField& U)
{
    ...
    // Apply correction to velocity field with given velocity mean/RMS profile
    forAll(cells_, i)
    {
        label cellI = cells_[i];
...
void Foam::fv::meanVelocityProfileSource::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldI
)
...
void Foam::fv::meanVelocityProfileSource::setValue
(
    fvMatrix<vector>& eqn,
    const label
)
